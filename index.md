# Operating Systems Coursework Journal  
**Student ID:** A00030810 
**Module:** CMPN202 Operating Systems  
**GitHub Pages Journal**

Welcome to my Operating Systems coursework journal. This site documents my progress across all 7 weeks, covering system configuration, security hardening, monitoring, performance testing, and final security audit.

---

## 📚 Table of Contents

- [Week 1 – System Planning & Architecture](week1.md)
- [Week 2 – Security Planning & Methodology](week2.md)
- [Week 3 – Application Selection & Monitoring Strategy](week3.md)
- [Week 4 – Initial Security Configuration](week4.md)
- [Week 5 – Advanced Security & Monitoring Scripts](week5.md)
- [Week 6 – Performance Testing & Analysis](week6.md)
- [Week 7 – Security Audit & Final Evaluation](week7.md)
- [Week 7 – security](security-audit-report.md)
- [IMAGE – IMAGE](image)


---

## 🔧 Systems Used

| System | OS Version | Purpose |
|--------|-------------|-----------|
| Ubuntu Desktop 24.04.3 (Workstation) | GUI | SSH administration, monitoring, testing |
| Ubuntu Server 24.04.3 (Headless) | CLI | Remote server for security configuration and monitoring |

---

## 📌 Overview

This project follows a **dual-system** architecture, using VirtualBox with a headless Ubuntu server accessible only through SSH. Weekly deliverables include screenshots, commands, configuration files, and reflections.

---

## 🔗 Navigation  
Use the links above to view each weekly entry.

